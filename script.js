...
function launchApp(type) {
    var cfg = appsList[type];
    if (!cfg) return;

    var existing = openWindows.find(function(w) { return w.type === type && !w.closed; });
    if (existing) {
...
function runCommand(cmd, output) {
    ...
    else if (l === 'about') line.textContent = 'JerOS v1.0 Modern Web OS';
...
function initCanvas() {
    ...
    window.addEventListener('resize', function() {
        var rect = canvas.getBoundingClientRect();
        var tempData = canvasContext.getImageData(0, 0, canvas.width, canvas.height);
        canvas.width = rect.width;
        canvas.height = rect.height;
...
function openFolderContent(folder, win) {
    ...
    for (var i = 0; i < items.length; i++) {
        ...
        var icon = items[i].includes('.') ? 'file' : 'folder';
        div.innerHTML = `
            <span class="file-icon">${icon === 'file' ? '📄' : '📁'}</span>
            <span class="file-name">${items[i]}</span>
        `;
    ...
    if (val === '=') {
        ...
        try {
            ...
            if (history) {
                ...
                var displayExpr = expression.replace(/\*/g, 'x').replace(/\//g, '/');
...
document.getElementById('tray-volume').addEventListener('click', function() {
    ...
    var icon = systemVolume === 0 ? 'muted' : (systemVolume > 60 ? 'high' : (systemVolume > 30 ? 'medium' : 'low'));
...
function closeWin(btn) {
    ...
    var data = openWindows.find(function(w) { return w.win === win; });
    if (data) {
        data.closed = true;
        win.classList.add('closing');
        setTimeout(function() {
            win.remove();
            openWindows = openWindows.filter(function(w) { return w.win !== win; });
            refreshTaskbar();
        }, 160);
    }
}
...
function closeWinFromTaskbar(btn) {
    ...
    var visibleWindows = openWindows.filter(function(w) { return !w.closed; });
